...
### Detection & Execution with `uv`

Prefer running Playwright ephemerally via `uv` without requiring global or environment installation:

```bash
uv run --with playwright python <tmp.py> <src.html> <out.png> [scale]
```

`uv run --with playwright` automatically fetches and isolates the Playwright dependency in milliseconds.

If `uv` is not present, fall back to standard Python:
```bash
python <tmp.py> <src.html> <out.png> [scale]
```

If neither is ready or Playwright fails to launch:
> To enable PNG export without manual setup, ensure `uv` is installed, or run:
> ```
