...
<!-- 3. Node name (Geist sans — human-readable, no redundant numeric prefixes) -->
<text x="CX" y="CY+2" fill="#2d3142" font-size="12" font-weight="600"
      font-family="'Geist', sans-serif" text-anchor="middle">Node Name</text>
<!-- 4. Technical sublabel (Geist Mono) -->
<text x="CX" y="CY+18" fill="#4f5d75" font-size="9"
      font-family="'Geist Mono', monospace" text-anchor="middle">tech:port</text>
```

### Badges and numbering — avoid redundant labels

- **Do NOT add generic step/stage/module badges** (e.g., `ETAPA 1`, `PASO 1`, `MÓDULO 01`, `ORIGEN`, `DESTINO`). They generate visual noise without providing semantic value.
- **Do NOT prefix card or node titles with sequential numbers** (e.g., use `Lectura de comandos` instead of `1. Lectura de comandos`) when arrows, pipes, or layout already express the sequence and hierarchy.
- Badges/type tags should only be used when they define actual technical interfaces, data types, or protocols (e.g., `TLS`, `LWP`, `API`), never generic counters or redundant step markers.
...
### Legend — optional and only when strictly necessary

**Do not add a bottom separator line or legend by default.** If all nodes and connectors are clearly labeled and self-explanatory in context (especially in reports, slides, or academic papers), omit the legend entirely. When a legend is explicitly required:
- Place it as a clean horizontal group with generous spacing.
- Never use heavy dividing lines if the diagram canvas is already bounded.
- Expand SVG `viewBox` height only if a legend is included.
